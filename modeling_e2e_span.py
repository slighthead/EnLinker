...
class DualEncoderBert(BertPreTrainedModel):
    ...
    def init_modules(self):
        ...
        for module in self.classifier1.modules():
            if isinstance(module, nn.Linear):
                module.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
            if isinstance(module, nn.Linear) and module.bias is not None:
                module.bias.data.zero_()
    ...
    def forward_1(self,
        ...
        last_layer = mention_outputs[1]

        logit = self.classifier1(last_layer)
        
        
        pair_loss = self.loss_bce(logit,target.resize(1,1))
        
        return pair_loss, logit